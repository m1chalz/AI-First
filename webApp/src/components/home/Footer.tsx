        ...
        <div className={styles.column}>
          ...
          <ul className={styles.linkList}>
            {FOOTER_QUICK_LINKS.map((link) => (
              <li key={link.id}>
                {link.isPlaceholder ? (
                  <span className={styles.placeholderLink} data-testid={`landing.footer.quickLink.${link.id}`}>
                    {link.label}
                  </span>
                ) : (
                  <Link to={link.href} className={styles.link} data-testid={`landing.footer.quickLink.${link.id}`}>
                    {link.label}
                  </Link>
                )}
              </li>
      ...
      <div className={styles.bottom}>
        ...
        <div className={styles.legalLinks}>
          {FOOTER_LEGAL_LINKS.map((link) => (
            <a key={link.id} href={link.href} className={styles.legalLink} data-testid={`landing.footer.legalLink.${link.id}`}>
              {link.label}
            </a>
          ))}
        </div>
