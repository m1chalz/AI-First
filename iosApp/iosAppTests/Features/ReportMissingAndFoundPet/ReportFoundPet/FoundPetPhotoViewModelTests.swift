...
@MainActor
final class FoundPetPhotoViewModelTests: XCTestCase {
    ...
    private var sut: FoundPetPhotoViewModel!
    
    override func setUp() {
        super.setUp()
        cache = PhotoAttachmentCacheFake()
        toastScheduler = ToastSchedulerFake()
        flowState = FoundPetReportFlowState(photoAttachmentCache: cache)
        photoSelectionProcessor = PhotoSelectionProcessorStub()
        sut = FoundPetPhotoViewModel(
            flowState: flowState,
    ...
    func testRestorePersistedAttachment_shouldUseFlowStateMetadata() async {
        ...
        let freshFlowState = FoundPetReportFlowState(photoAttachmentCache: cache)
