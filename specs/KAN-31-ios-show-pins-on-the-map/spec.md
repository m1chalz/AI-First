...
**Feature Branch**: `KAN-31-ios-show-pins-on-the-map`  
**Created**: 2025-12-19  
**Status**: Draft  
...
A user opens the iOS landing page with location permission granted and wants to see at a glance where missing pets were last seen in their area. The map preview displays visual pins at the approximate locations of missing pet announcements within the represented ~10 km radius.
...
**Independent Test**: Can be tested by opening the landing page with location permission granted and missing pet announcements in the database within 10 km radius. Verify that pins appear on the map preview at the correct approximate locations.

**Acceptance Scenarios**:

1. **Given** the landing page is displayed with location permission granted, **When** missing pet announcements exist within the ~10 km radius of user's location, **Then** pins are visible on the map preview at the approximate last-seen locations of those pets
...
- **FR-001**: The map preview MUST display visual pins for missing pet announcements that fall within the represented ~10 km radius around the user's current location
...
- **FR-006**: When no missing pet announcements exist within the represented area, the map preview MUST be displayed without pins and without an empty-state message
...
- **FR-012**: This specification MUST be implemented for iOS only (Android and Web are explicitly out of scope)

### Key Entities *(include if feature involves data)*

- **Missing Pet Announcement**: A report of a missing animal including last-seen coordinates (latitude/longitude), identification details (photo, name, species), last-seen date, and owner contact information
...
- The backend API provides missing pet announcements with coordinates that fall within the user's ~10 km radius
...
- **Backend API**: Requires endpoint to fetch missing pet announcements filtered by geographic area (lat/lon + radius)
...
- Q: How many announcements should be loaded? → A: All announcements within the ~10 km radius (backend handles filtering)
