...
- **FR-002a**: The "Date of disappearance" field MUST be the first form field displayed after the title and subtitle, default to today's date (or the last saved value) and open the Android DatePickerDialog configured so users MAY pick any past date (including today) but future dates are disabled and cannot be selected.
- **FR-003**: "Pet name (optional)" MUST be displayed after the date field, implemented as an OutlinedTextField; empty state is valid and does not block Continue.  
