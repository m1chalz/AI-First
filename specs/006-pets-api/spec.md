...
- Q: What validation should be applied to contactEmail and contactPhone fields? → A: Basic format validation (simple email regex, phone contains digits)
...
- **Missing Optional Fields**: Optional fields (like photoUrl) are included in response with `null` value when not present
...
- **FR-011**: System MUST include optional fields (like photoUrl) in response with `null` value when not present in the database
- **FR-012**: System MUST ignore unexpected or malformed query parameters and process requests normally
- **FR-013**: System MUST validate contactEmail using basic email format validation (simple regex pattern) and contactPhone must contain digits
- **FR-014**: System MUST allow public access to the endpoint without requiring authentication or authorization
- **FR-015**: System MUST validate petType field to be one of the allowed enum values: DOG, CAT, BIRD, RABBIT, OTHER
- **FR-016**: System MUST return announcements in database default order with no specific sorting applied
...
- **SC-005**: Each announcement includes all required fields with valid data types (UUID for id, ISO 8601 UTC format for lastSeenDate: "YYYY-MM-DDTHH:MM:SSZ", etc.)
