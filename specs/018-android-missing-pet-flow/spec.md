...
- What happens when user rotates device during the flow (should state survive configuration changes)?
...
- What happens when system back button is pressed vs. in-app back navigation?
...
- Photo capture/selection uses standard Android photo picker or camera intent
...
- No autosave functionality - user must complete flow in one session (though data survives configuration changes like rotation)
- Implementation will use Jetpack Compose for UI
- ViewModel will handle state management and survive configuration changes
...
- Android photo picker/camera access permissions (assumed to be requested appropriately)
- Navigation framework setup in Android app (Jetpack Navigation Component)
