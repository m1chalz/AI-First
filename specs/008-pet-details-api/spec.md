...
- **Non-Existent Pet ID**: System returns HTTP 404 with structured error response containing code "NOT_FOUND" and message indicating the pet announcement was not found
...
- **FR-005**: System MUST accept UUID string format as the ID parameter (e.g., "123e4567-e89b-12d3-a456-426614174000")
...
- **FR-008**: System MUST include optional fields (photoUrl, breed, email, locationRadius) in response with `null` value when not present in the database
- **FR-009**: System MUST allow public access to the endpoint without requiring authentication or authorization
- **FR-010**: System MUST return error response for HTTP 404 with structure: `{ error: { code: "NOT_FOUND", message: "Pet announcement not found" } }`
...
- **SC-001**: Clients can retrieve a specific pet announcement in under 500 milliseconds
