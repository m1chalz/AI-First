...
Users can quickly understand the portal's four main capabilities through visually distinct feature cards in the hero section: Search Database, Report Missing, Found a Pet, and Location Based features.

**Why this priority**: Feature cards provide immediate clarity about what users can do on the portal. This reduces confusion and helps users quickly identify their next action.
...
Users can view up to 5 most recently reported lost pets in a dedicated section below the hero, with each pet card showing photo, location, pet type/breed, and report date. Users can click "View all →" to see the complete list.
...
1. **Given** there are 5 or more lost pet announcements, **When** user views the landing page, **Then** exactly 5 most recent lost pets are displayed in card format
2. **Given** there are fewer than 5 lost pet announcements, **When** user views the landing page, **Then** all available lost pets are displayed
3. **Given** a pet card is displayed, **When** user views it, **Then** they see pet photo, status badge (MISSING/FOUND), location with distance, pet type and breed, and report date
...
3. **Given** the footer is displayed, **When** user views quick links, **Then** they see links for "Report Lost Pet", "Report Found Pet", and "Search Database"
4. **Given** the footer is displayed, **When** user views contact information, **Then** they see email, phone number, and physical address
5. **Given** the footer is displayed, **When** user views the bottom section, **Then** they see copyright notice "© 2025 PetFinder. All rights reserved." and legal links (Privacy Policy, Terms of Service, Cookie Policy)

---

### Edge Cases

- **What happens when there are no lost pet announcements?**: Display an empty state message "No recent lost pet reports. Check back soon!" in the recently lost pets section
...
- **What happens when the backend API is unavailable?**: Show an error message in the recently lost pets section: "Unable to load recent pets. Please try again later."
- **What happens on narrow mobile screens?**: Feature cards stack vertically, pet cards display in a scrollable horizontal row or stack vertically, footer columns stack vertically
...
- **FR-006**: Each feature card MUST display a colored icon background, feature title, and brief description
- **FR-007**: Landing page MUST display a "Recently Lost Pets" section with heading, subtitle, and "View all →" link
- **FR-008**: Recently Lost Pets section MUST fetch and display up to 5 most recent lost pet announcements from the backend API
- **FR-009**: Each pet card MUST display: pet photo, status badge (MISSING in red or FOUND in blue), location name with distance, pet type and breed separated by a dot, and report date with calendar icon
...
- **FR-014**: Footer quick links column MUST display links for "Report Lost Pet", "Report Found Pet", and "Search Database"
...
- **FR-017**: Landing page MUST be responsive and adapt to mobile (320px+), tablet (768px+), and desktop (1024px+) screen sizes
- **FR-018**: Landing page MUST display loading indicators while fetching pet announcements
- **FR-019**: Landing page MUST handle API errors gracefully with user-friendly error messages

### Key Entities

- **Pet Announcement**: Represents a reported lost or found pet. Key attributes: photo URL, status (MISSING/FOUND), location name, coordinates, distance from user, pet type, breed, report date, owner contact
...
- **SC-002**: When 5 or more lost pets exist, exactly 5 most recent pets are displayed; when fewer exist, all available pets are shown
- **SC-003**: Landing page maintains readable layout on screen widths from 320px (mobile) to 1920px+ (desktop) without horizontal scrolling
- **SC-004**: Users can identify the portal's primary purpose within 5 seconds of viewing the hero section
- **SC-005**: All interactive elements (pet cards, links, buttons) provide visual feedback on hover/focus
...
- **Backend API**: Backend provides an endpoint that returns recently lost pets sorted by date (newest first). The endpoint accepts a limit parameter to request up to 5 pets
...
- **Error Handling**: API errors are caught and displayed to users with retry options where appropriate
