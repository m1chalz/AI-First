...
- What happens when the repository fails to fetch pet details? (Show error state with retry option)
- What happens when API returns ACTIVE status instead of MISSING? (Map ACTIVE → MISSING in the data layer for consistent UI display)
...
- **FR-016**: All interactive elements MUST have accessibility identifiers in the format "petDetails.element"
...
- **PetDetails**: Represents all information about a pet including identification data, location, contact information, and descriptive details. Key attributes include:
  ...
  - Status (required: MISSING, FOUND, or CLOSED) → maps to API `status` (note: API uses ACTIVE instead of MISSING)
