...
- **FR-021**: Existing iOS `AnimalRepositoryProtocol` MUST be extended with a `getPetDetails(id: String) async throws -> PetDetails` method (no separate `PetRepository` type); its Phase 1 implementation (`AnimalRepository`) WILL return mocked data for development (backend endpoint `GET /api/v1/announcements/:id` already exists, but integration is deferred to Phase 2)
