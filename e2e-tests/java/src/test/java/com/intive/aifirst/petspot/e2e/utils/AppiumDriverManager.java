...
public class AppiumDriverManager {
    ...
    /** iOS Simulator UUID - iPhone 16 */
    private static final String IOS_SIMULATOR_UUID = "F437ECEB-E302-4400-B91A-BBA472E7E12D";
    ...
    /**
     * Sets iOS simulator GPS location via simctl.
     * Uses format: xcrun simctl location <UUID> set <lat>,<lon>
     * 
     * @param latitude GPS latitude
     * @param longitude GPS longitude
     */
    private static void setIOSLocationViaSimctl(double latitude, double longitude) {
        try {
            // Format: "lat,lon" as single argument (e.g., "1.0,1.0" or "51.1,17.0")
            // Note: Using 1.0,1.0 instead of 0.0,0.0 due to Appium bug with zero coordinates
            String latLon = latitude + "," + longitude;
            
            // Use hardcoded UUID for iPhone 16 simulator
            ProcessBuilder pb = new ProcessBuilder(
                "xcrun", "simctl", "location", IOS_SIMULATOR_UUID, "set", latLon
            );
            pb.redirectErrorStream(true);
            Process process = pb.start();
            ...
            if (exitCode == 0) {
                System.out.println("iOS: ✅ Set GPS location via simctl (" + IOS_SIMULATOR_UUID + "): " + latitude + ", " + longitude);
            } else {
                System.out.println("iOS: ❌ simctl location set failed (exit code: " + exitCode + ")");
                if (output.length() > 0) {
                    System.out.println("iOS: simctl output: " + output.toString().trim());
                }
            }
        } catch (Exception e) {
            System.out.println("iOS: simctl location set error: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public static void restartApp() {
        ...
        System.out.println("Restarting app (full restart): " + appId);
        
        // Full restart: terminate app completely (kill process) then launch fresh
        if (appiumDriver instanceof AndroidDriver androidDriver) {
            androidDriver.terminateApp(appId);
            try {
                Thread.sleep(1500); // Wait for app to fully terminate
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
            // Verify app is terminated (log warning if not)
            if (androidDriver.queryAppState(appId) != ApplicationState.NOT_RUNNING) {
                System.err.println("Warning: App still running after terminate - state: " + androidDriver.queryAppState(appId));
            }
            ...
            // Verify app is terminated (log warning if not)
            if (iosDriver.queryAppState(appId) != ApplicationState.NOT_RUNNING) {
                System.err.println("Warning: App still running after terminate - state: " + iosDriver.queryAppState(appId));
            }
    ...
    public static void installApp() {
        ...
        try {
            ...
            // For iOS with @location tag: grant permission AFTER install (simctl requires installed app)
            if (appiumDriver instanceof IOSDriver && grantLocationPermission.get()) {
                System.out.println("iOS: Granting location permission after install (post-uninstall)...");
                grantIOSLocationPermission();
                Thread.sleep(500); // Give system time to apply permission
            }
    ...
    public static void setDeviceLocation(double latitude, double longitude) {
        ...
        if (appiumDriver instanceof AndroidDriver androidDriver) {
            ...
            System.out.println("Android: Device location set to: " + latitude + ", " + longitude);
        } else if (appiumDriver instanceof IOSDriver iosDriver) {
            // iOS: Use standard setLocation() API (deprecated but should work)
            try {
                org.openqa.selenium.html5.Location location = 
                    new org.openqa.selenium.html5.Location(latitude, longitude, 0);
                
                // Suppress deprecation warning - this is the most reliable method
                @SuppressWarnings("deprecation")
                IOSDriver suppressWarnings = iosDriver;
                suppressWarnings.setLocation(location);
                
                System.out.println("iOS: ✅ Device location set via setLocation(): " + latitude + ", " + longitude);
                
                // Give simulator time to register new location
                Thread.sleep(1000);
                
            } catch (Exception e) {
                System.out.println("iOS: ⚠️  setLocation() failed: " + e.getMessage());
                System.out.println("iOS: This may be an Appium/XCUITest limitation");
            }
        } else {
