# Missing Pet Report Flow - Android E2E Tests
# This feature tests the complete missing pet report flow on Android platform
# Uses Appium with Compose testTag accessibility identifiers

@mobile @android @missing-pet
Feature: Missing Pet Report Flow (Android)
  As a pet owner
  ...
  Background:
    Given I have launched the Android app
  ...
  @android @us1
  Scenario: All Interactive Elements Have Correct Test Tags
    When I tap the "report missing animal" button on animal list
    Then the screen should have accessibility identifier "missingPet.microchip.continueButton" for continue button
    When I tap the "continue" button
    Then the screen should have accessibility identifier "animalPhoto.continue" for continue button
    When I tap the "continue" button
  ...
  Scenario: Exit Flow from Step 1 by Tapping Back Button
    ...
    # Verify: Flow exits and animal list screen displays
