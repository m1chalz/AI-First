 ...
 */
export default async function announcementAuthMiddleware(req: RequestWithBasicAuth, _res: Response, next: NextFunction): Promise<void> {
  const announcementId = req.params.id;

  // Get password hash from database
  const row: PasswordRow | undefined = await db('announcement').select('management_password_hash').where('id', announcementId).first();
  ...
  const passwordMatch = await verifyPassword(req.basicAuth?.password ?? '', row.management_password_hash);
